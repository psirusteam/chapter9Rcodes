...
library(knitr)
readRDS("data/tablas/secc3/tab_01.rds")  %>% 
  kable(caption = "Estimated population by place of residence, Ethiopia, 2018/2019")
```
...
```{r, eval=FALSE, echo=FALSE}
library(tidyr)
readRDS("data/tablas/secc3/tab_02.rds") %>% 
select(Age_Group, prop) %>%
  pivot_wider(
    names_from = Age_Group,
    values_from = prop,
    values_fill = list(n = 0)
  ) %>% select(Region, `0 - 5` , `0 - 9`,  `0 - 14`, `15 - 64`, `65 +` ) %>% 
  kable(caption = "Estimate by age group and place of residence, Ethiopia, 2018/2019")
```
...
```{r, eval=FALSE, echo=FALSE}
readRDS("data/tablas/secc3/tab_02.rds") %>% 
select(Region, Age_Group, prop_se) %>%
   mutate(prop_se = round(prop_se * 100, 2)) %>% 
  pivot_wider(
    names_from = Age_Group,
    values_from = prop_se,
    values_fill = list(n = 0)
  ) %>% select(Region, `0 - 5` , `0 - 9`,  `0 - 14`, `15 - 64`, `65 +` )  %>% 
  kable(caption = "Estimated standard deviation by age group by place of residence, Ethiopia, 2018/2019")
